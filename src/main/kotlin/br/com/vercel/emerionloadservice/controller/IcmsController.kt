...
class IcmsController(
    ...
    @GetMapping("all")
    fun getAllIcms(@PageableDefault(size = 40) pageable: Pageable): Page<IcmsIngestionDto> {
        return icmsService.getAllIcms(pageable).map { it.toIngestionDto(companyProvider.getCompanyCnpj()) }
    }
