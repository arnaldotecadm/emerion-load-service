...
class CustomerAddressController(
    ...
    ): Page<CustomerAddressIngestionDto> =
        this.customerAddressService.getAllAddresses(pageable).map {
            it.toIngestionDto(companyProvider.getCompanyCnpj())
        }

    @GetMapping("{codCli}")
    fun getAddressByCodCli(
        @PathVariable codCli: Long,
    ): CustomerAddressIngestionDto = this.customerAddressService.getAddressByCodCli(codCli).toIngestionDto(companyProvider.getCompanyCnpj())

    @PostMapping("{codCli}/send")
    fun sendAddressToIngestion(
        @PathVariable codCli: Long,
    ): ResponseEntity<Void> {
