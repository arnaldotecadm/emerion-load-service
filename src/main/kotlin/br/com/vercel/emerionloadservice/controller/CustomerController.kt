...
class CustomerController(
    ...
    ): Page<CustomerIngestionDto> =
        this.customerService.getAllCustomers(pageable).map { it.toIngestionDto(companyProvider.getCompanyCnpj()) }

    @GetMapping("{codCli}")
    fun getCustomerByCodCli(
        @PathVariable codCli: Long,
    ): CustomerIngestionDto = this.customerService.getCustomerByCodCli(codCli).toIngestionDto(companyProvider.getCompanyCnpj())

    @PostMapping("{codCli}/send")
    fun sendCustomerToIngestion(
        @PathVariable codCli: Long,
    ): ResponseEntity<Void> {
