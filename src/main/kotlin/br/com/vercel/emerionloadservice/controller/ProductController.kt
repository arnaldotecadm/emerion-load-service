...
import br.com.vercel.emerionloadservice.api.model.ProductIngestionDto
...
class ProductController(
    ...
    ): Page<ProductIngestionDto> = this.productService.getAllProducts(pageable).map { it.toIngestionDto(companyProvider.getCompanyCnpj()) }

    @GetMapping("{id}")
    fun getProductById(
        @PathVariable id: String,
    ): ProductIngestionDto = this.productService.getProductById(id).toIngestionDto(companyProvider.getCompanyCnpj())

    @PostMapping("{id}/send")
    fun sendProductToIngestion(
        @PathVariable id: String,
    ): ResponseEntity<Void> {
