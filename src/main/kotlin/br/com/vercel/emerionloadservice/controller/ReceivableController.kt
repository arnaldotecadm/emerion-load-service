...
class ReceivableController(
    ...
    @GetMapping("all")
    fun getAllReceivables(
        @PageableDefault(size = 40) pageable: Pageable,
    ): Page<ReceivableIngestionDto> =
        receivableService.getAllReceivables(pageable).map { it.toIngestionDto(companyProvider.getCompanyCnpj()) }

    @GetMapping("{codCli}")
    fun getReceivablesByCodCli(
        @PathVariable codCli: Long,
    ): List<ReceivableIngestionDto> =
        receivableService
            .getReceivablesByCodCli(codCli)
            .map { it.toIngestionDto(companyProvider.getCompanyCnpj()) }

    @PostMapping("{codCli}/send")
    fun sendReceivablesToIngestion(
        @PathVariable codCli: Long,
    ): ResponseEntity<Void> {
