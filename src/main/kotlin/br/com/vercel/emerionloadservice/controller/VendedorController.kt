...
import br.com.vercel.emerionloadservice.api.model.VendedorIngestionDto
...
class VendedorController(
    ...
    ): Page<VendedorIngestionDto> =
        this.vendedorService
            .getAllVendedores(pageable)
            .map { it.toIngestionDto(companyProvider.getCompanyCnpj()) }

    @GetMapping("{codVen}")
    fun getVendedorByCodVen(
        @PathVariable codVen: Long,
    ): VendedorIngestionDto = this.vendedorService.getVendedorByCodVen(codVen).toIngestionDto(companyProvider.getCompanyCnpj())
